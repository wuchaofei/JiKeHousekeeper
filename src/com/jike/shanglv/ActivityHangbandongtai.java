...
public class ActivityHangbandongtai extends Activity {
	...
	private TextView singleline_tv, doubleline_tv, startcity_tv, endcity_tv,
			startcity_code_tv, endcity_code_tv, startoff_date_tv;
	private LinearLayout startcity_choose_ll, endcity_choose_ll;
	private RelativeLayout date_choose_single_rl, flight_no_rl, city_choose_rl;
	private ImageView scrollbar_iv, swith_city_iv;
	private ImageButton back_imgbtn, home_imgbtn;
	private com.jike.shanglv.Common.ClearEditText flightno_cet;
	...
	private SingleOrDouble wayType = SingleOrDouble.singleWay; // 单程:按起降城市 or
																// 往返：按航班号
	...
	private void initView() {
		...
		flight_no_rl = (RelativeLayout) findViewById(R.id.flight_no_rl);
		city_choose_rl = (RelativeLayout) findViewById(R.id.city_choose_rl);
		flightno_cet = (ClearEditText) findViewById(R.id.flightno_cet);
	}
			...
			try {
				...
				case R.id.chongzhi_button:// 搜索
					...
					if (wayType == SingleOrDouble.doubleWayGo
							&& flightno_cet.getText().toString().trim()
									.length() == 0) {
						// new
						// AlertDialog.Builder(context).setTitle("请输入要查询的航班号码").setPositiveButton("确定",
						// null).show();
						final CustomerAlertDialog cad = new CustomerAlertDialog(
								context, true);
					...
					Intent intents = new Intent(context,
							ActivityHangbandongtaiSearchlist.class);
					intents.putExtra("wayType", wayType);
					intents.putExtra("flightNo", flightno_cet.getText()
							.toString().trim());
